...
function callbackfunc(network, loggedIn) {
    var id = loggedIn ? 'loggedIn' : 'notLoggedIn';
    var favicon = faviconUri(network);
    var url = network.domain + network.redirect;
    var el = '<a class="btn btn-lg" target="_blank" href="' + url + '" target="_blank" class=network><img src=' + favicon + '><span>' + network.name + '</span></a><br>';
    if (loggedIn) {
        socialresult = document.getElementById("socialmedia");
        socialresult.innerHTML += el;
        console.log("yaya" + network.name);
    } else {
        ///console.log("meh" + network.name);
    }
}
leakSocialMediaAccounts(callbackfunc);
